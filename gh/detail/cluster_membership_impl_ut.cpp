...
/// @test Verify that cluster membership works correctly even when one of the cluster members returns an empty list.
TEST(cluster_membership_impl, with_empty_list) {
  using namespace std::chrono_literals;
  using namespace gh::detail;
  using namespace testing;
  ...
  // ... simulate an empty list in the results ...
  auto& call0 = EXPECT_CALL(*queue.interceptor().shared_mock, async_rpc(Truly([](auto op) {
    return op->name == "cluster_membership/member_list";
  })));
  call0.WillOnce(Invoke([](auto bop) {
  ...
  EXPECT_CALL(*queue.interceptor().shared_mock, async_rpc(Truly([](auto op) {
    return op->name == "cluster_membership/member_list";
  }))).After(call0).WillOnce(Invoke([&cluster](auto bop) {
    using op_type = cluster_membership_type::member_list_op;
    auto* op = dynamic_cast<op_type*>(bop.get());
    ASSERT_TRUE(op != nullptr);

    // ... the collection should have changed as the previous callback executed ...
    auto urls = cluster.current_urls();
    EXPECT_THAT(urls, ElementsAre("url0.0", "url0.1", "url0.2"));

    // ... in this test we just assume everything worked, so provide a response ...
    auto& m0 = *op->response.add_members();
    *m0.add_clienturls() = std::string("url1.0");
    *m0.add_clienturls() = std::string("url1.1");
    // ... and to not forget the callback ...
    bop->callback(*bop, true);
  }));
  ...
  ASSERT_TRUE(pt);
  pt->callback(*pt, true);
  urls = cluster.current_urls();
  EXPECT_THAT(urls, ElementsAre("url1.0", "url1.1"));

  EXPECT_NO_THROW(cluster.shutdown());
}

/// @test Verify that cluster membership raises the right exceptions when provided with invalid arguments.
TEST(cluster_membership_impl, invalid_arguments) {
  using namespace std::chrono_literals;
  using namespace gh::detail;
  using namespace testing;
