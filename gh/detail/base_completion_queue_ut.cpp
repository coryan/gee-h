...
/**
 * @test Verify that gh::completion_queue internal loop can handle timeouts properly.
 */
TEST(base_completion_queue, run_timeout) {
  gh::detail::base_completion_queue queue;
  // run the event loop in a separate thread ...
  std::promise<void> start;
  std::promise<void> end;
  ...
  std::this_thread::sleep_for(3 * gh::detail::base_completion_queue::loop_timeout);
  queue.shutdown();
  std::this_thread::sleep_for(gh::detail::base_completion_queue::loop_timeout);
