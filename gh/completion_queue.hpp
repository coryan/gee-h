...
public:
  ...
  void async_rpc(C* async_client, M C::*call, W&& request, std::string name, Functor&& f) {
    ...
    static_assert(
        requirements::matches::value, "The member function signature does not match: "
    ...
    static_assert(std::is_same<W, request_type>::value, "Mismatch request parameter type vs. operation signature");
  ...
  std::shared_future<std::shared_ptr<typename detail::async_stream_create_requirements<M>::stream_type>>
  async_create_rdwr_stream(C* async_client, M C::*call, std::string name, use_future) {
    using ret_type = std::shared_ptr<typename detail::async_stream_create_requirements<M>::stream_type>;
    auto promise = std::make_shared<std::promise<ret_type>>();
    this->async_create_rdwr_stream(async_client, call, name, [promise, name](auto stream, bool ok) {
      // intercept canceled operations and raise an exception ...
      if (not ok) {
        std::string err = "async create_rdwr_stream cancelled for " + name;
  ...
  async_writes_done(detail::async_rdwr_stream<W, R> const& stream, std::string name, use_future) {
    auto promise = std::make_shared<std::promise<void>>();
    this->async_writes_done(stream, name, [promise](auto& op, bool ok) {
      if (not ok) {
        promise->set_exception(std::make_exception_ptr(std::runtime_error("async writes done cancelled")));
        return;
      }
      promise->set_value();
    });
   ...
   */
  template <typename W, typename R>
  std::shared_future<grpc::Status>
  async_finish(detail::async_rdwr_stream<W, R> const& stream, std::string name, use_future) {
    auto promise = std::make_shared<std::promise<grpc::Status>>();
    this->async_finish(stream, name, [promise](auto& op, bool ok) {
      if (not ok) {
        promise->set_exception(std::make_exception_ptr(std::runtime_error("async finished cancelled")));
        return;
      }
